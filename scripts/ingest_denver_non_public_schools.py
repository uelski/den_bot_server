"""ingest_denver_non_public_schools.py

Read the Denver non-public schools geojson and ingest one Document per school
into the existing denver_gis_catalog Qdrant collection. Templated NL summaries
— no LLM calls.

Source: Denver Open Data Catalog "Non-Public School 2025-2026" dataset
  https://opendata-geospatialdenver.hub.arcgis.com/datasets/geospatialDenver::non-public-school-2025-2026/about
The geojson is gitignored; download manually into data/ before running.

Field naming notes:
  - The source SCHOOL_TYPE column ("Parochial" / "Independent") is preserved
    as `metadata.school_type` to match the source schema 1:1.
  - Our own dataset-origin discriminator is stored separately as
    `metadata.institution_type = "non-public"`. The companion public-schools
    ingest will write `institution_type = "public"` to the same shared
    `doc_type = "denver_school"`. Frontends / retrieval can filter by either.

URL plumbing follows the libraries/parks pattern:
  - service_name = "Denver Non-Public Schools" (constant — sources panel
    collapses to one citation entry per dataset).
  - base_url = hub_url = dataset hub /about page.
  - display_name = SCHOOL_NAME → unique map_viewer labels per school.
  - map_url = per-school Google Maps deep link from geometry coordinates
    (Point geometry, [lon, lat] order — same as libraries).

Run:
  python scripts/ingest_denver_non_public_schools.py            # append (refuses if docs exist)
  python scripts/ingest_denver_non_public_schools.py --purge    # delete prior docs first
"""
...
GEOJSON_GLOB = "ODC_EDU_NONPUBLICSCHOOLS_P_*.geojson"

QDRANT_URL = os.getenv("QDRANT_URL", "http://localhost:6333")
QDRANT_API_KEY = os.getenv("QDRANT_API_KEY")
COLLECTION_NAME = os.getenv("QDRANT_COLLECTION_NAME", "denver_gis_catalog")

SERVICE_NAME = "Denver Non-Public Schools"
DOC_TYPE = "denver_school"
INSTITUTION_TYPE = "non-public"
...
DATASET_HUB_URL = (
    ...
    "geospatialDenver::non-public-school-2025-2026/about"
)
...
def _clean(value) -> str | None:
    ...
    as None. (No "<Null>" sentinel here unlike rec centers.)"""
    if value is None:
        return None
    s = str(value).strip()
    return s or None
...
def _article_for(word: str) -> str:
    """Return 'an' before vowel-sound first letters, else 'a'. Heuristic
    rather than perfect — works cleanly for the two known SCHOOL_TYPE
    values ('Independent' → 'an', 'Parochial' → 'a')."""
    if not word:
        return "a"
...
def build_lead_sentence(props: dict) -> str:
    """Name + institution type + address. Always returns a sentence —
    falls back gracefully when school_type or address is missing."""
    name = _clean(props.get("SCHOOL_NAME")) or "School"
    ...
    if school_type:
        ...
        descriptor = "a non-public school"
...
def build_grades_sentence(props: dict) -> str | None:
    """Combine GRADE_LEVELS and NUM_STUDENTS into one sentence. Drops the
    enrollment clause when num_students is missing or zero. Returns None
...
def build_school_document(feature: dict) -> Document | None:
    ...
    Point geometry we need for the per-school map URL. Coordinates come
    from feature.geometry.coordinates ([lon, lat]), like libraries."""
...
def _doc_filter() -> Filter:
    """Filters to non-public school docs only — matches both doc_type and
    institution_type so the public-schools ingest's docs are unaffected by
    a --purge run on this script."""
    return Filter(
        must=[
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            f"{INSTITUTION_TYPE!r} before insert. Public-school docs are unaffected."
        ),
    )
    ...
    if geojson_path is None:
        logger.error(
            "no non-public schools geojson found in %s matching %s",
    ...
    logger.info("loaded %d non-public school features", len(features))

    docs = build_documents(features)
    logger.info(
        "built %d non-public school documents (skipped %d for missing geometry)",
    ...
    if existing > 0:
        if args.purge:
            ...
            logger.error(
                "found %d existing non-public school points in %s — re-run with --purge to replace",
